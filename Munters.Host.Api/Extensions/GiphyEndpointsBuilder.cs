...
internal static class GiphyEndpointsBuilder
{
    public static IEndpointRouteBuilder MapGiphyEndpoints(this IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet("/trending",
            async (IRequestHandler<TrendingQuery, Result<SearchQueryResult>> handler,
        ...
        endpoints.MapGet("/search/{text}",
            async (string text, IRequestHandler<SearchQuery, Result<SearchQueryResult>> handler,
                CancellationToken cancellationToken) =>
            {
